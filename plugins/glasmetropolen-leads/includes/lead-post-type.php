...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'glasmetropolen-leads' ),
					'singular_name' => __( 'Lead', 'glasmetropolen-leads' ),
					'edit_item'     => __( 'Förfrågan', 'glasmetropolen-leads' ),
					'search_items'  => __( 'Search leads', 'glasmetropolen-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'glasmetropolen-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'glasmetropolen-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'glasmetropolen-leads' ),
			'npl_phone'    => __( 'Phone', 'glasmetropolen-leads' ),
			'npl_service'  => __( 'Needs', 'glasmetropolen-leads' ),
			'npl_timeline' => __( 'When', 'glasmetropolen-leads' ),
			'npl_zip'      => __( 'ZIP', 'glasmetropolen-leads' ),
			'npl_source'   => __( 'Source', 'glasmetropolen-leads' ),
			'npl_status'   => __( 'Status', 'glasmetropolen-leads' ),
			'date'         => __( 'Received', 'glasmetropolen-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'glasmetropolen-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'glasmetropolen-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'glasmetropolen-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'glasmetropolen-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'glasmetropolen-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'glasmetropolen-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'glasmetropolen-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'glasmetropolen-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'glasmetropolen-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'glasmetropolen-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'glasmetropolen-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'glasmetropolen-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
