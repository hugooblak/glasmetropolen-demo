...
add_filter(
	...
	function ( $exporters ) {
		$exporters['glasmetropolen-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'glasmetropolen-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'glasmetropolen-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'glasmetropolen-leads',
						'group_label' => __( 'Quote requests', 'glasmetropolen-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['glasmetropolen-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'glasmetropolen-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
