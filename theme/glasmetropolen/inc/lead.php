 ...
 * or photo. They call gl_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Glasmetropolen.
 *
 ...
 * @return string
 */
function gl_sample_note() {
	return gl_lead( 'ar_riktig' ) ? '' : ' <span class="gl-sample">(platshållare)</span>';
}
 ...
 * @return string File name for gl_lead_img().
 */
function gl_lead_bild( $i ) {
	$bilder = (array) gl_lead( 'bilder', array() );
 ...
 * @return string
 */
function gl_ring_label() {
	$namn = gl_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, gl_lead( 'telefon' ) ) : 'Ring ' . gl_lead( 'telefon' );
}
